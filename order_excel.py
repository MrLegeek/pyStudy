...
class ExcelData():
    ...
    def readExcel(self):
        ...
        mysql_str = "insert into test (client,girard,color,size,num,price) values "
        if self.rowNum < 2:
            return 'error excle 数据行数小于2!'
        else:
            for i in range(1, self.rowNum):  # 从第二行（数据行）开始取数据

                mysql_str += "('" + self.table.row_values(i)[0] + "','" + self.table.row_values(i)[2] + "','" \
                             + self.table.row_values(i)[3] + "','" + self.table.row_values(i)[4] + "','" \
                             + str(self.table.row_values(i)[5]) + "','" + str(self.table.row_values(i)[6]) + "'),"

        with open('./clientOrder.sql', 'w') as fileobject:  # 使用‘w’的方式打开
            fileobject.write(mysql_str.strip(','))  # 去掉末尾的,
